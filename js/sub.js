...
document.addEventListener('DOMContentLoaded', () => { // Html이 완전히 화면 로딩 후 실행
  ...
  const qsa = (sel, ctx = document) => [...ctx.querySelectorAll(sel)];// 선택자에 해당하는 모든 요소를 배열로 변환
  ...
  // /\D/g(정규식)은 숫자가 아닌 문자를 모두 제거함
  // qs, / qsa : 유틸함수 (다른 요소들에도 활용 가능하며, HTML 접근 코드를 간결하게 유지 해주는 역할)

  /* max-length 300 초과하지 않고 입력필드 글자수 제한 + 실시간 카운트 */
  const inquireInput = qs('#inquire');

  if (inquireInput) {
    const counterEl = qs('.word_count .current'); // 현재 글자수를 실시간 표시
  ...
  // .lock이 css에서 반드시 overflow:hidden 으로 설정되어 있어야 효과 <- 아니면 안 열림
  ...
  // 업로드 가능한 최대 용량 30MB x 1024 x 1024byte
  ...
  qsa('.btn_submit').forEach(btn => {
    btn.addEventListener('click', async e => {
      ...
      const must = (name, msg) => {
        ...
        return el.value.trim(); //값이 존재하면 재거 후 반환
      };

      // try   { 입력값의 유효성 검사, FormData 생성 및 서버 전송, 응답결과 처리 }
      // catch { 필수 입력값 누락 throw로 중단하고 아무 처리안함 } 
      try {
        must('name',    '이름을 입력해 주세요'  );
        must('company', '업체명을 입력해 주세요' );
        must('phone',   '연락처을 입력해 주세요' );

        const email = must('email', '이메일을 입력해 주세요');
        if (!emailCheck(email)) { alert('이메일을 확인해 주세요'); get('email')?.focus(); return;}

        must('inquire', '문의 내용을 입력해 주세요')
        if (!provacyChk?.checked) {alert('개인정보 이용정책을 동의해주세요'); return; }

        const fd = new FormData(form); // FormData: 내부 모든 입력값을 자동 수집
        ...
        if (json.SUCCESS) {
          qs('.comlete_pop')?.classList.add('active');
          lockBody();
        } else {
          alert('등록에 실패했습니다.')
        }
