...
const visualSlide = new Swiper('.visual_slide', {
  ...
  on: {
        progress() { // 슬라이드가 움직이는 중 배경 위치 이동(.bg)
          this.slides.forEach(slide => {
            const slideProgress = slide.progress;
            const innerOffset = this.width * 0.5;
            const innerTranslate = slideProgress * innerOffset;
            slide.querySelector('.bg').style.transition = `translate3d(${innerTranslate}pc, 0, 0)`;
          });
        },
        setTransition(swiper, speed) { // 슬라이드 전환ㅅ 애니메이션 시간 설정
          this.slides.forEach(slide => {
...
$('.link_open').on('click', function(){
  ...
  const isOpen = $this.attr('title') === langOpen; //현재 상태가 열려있는가
